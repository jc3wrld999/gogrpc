...
.DEFAULT_GOAL := help
.PHONY: greet blog calculator process help
project := greet calculator blog process
...
$(project):
	@${CHECK_DIR_CMD}
	protoc $@/${PROTO_DIR}/*.proto --go_opt=module=${PACKAGE} --go_out=. --go-grpc_opt=module=${PACKAGE} --go-grpc_out=.
	$(SETENV) env GOOS=${os} $(SETENV) env GOARCH=${arch} go build -o ${BIN_DIR}/$@/${SERVER_BIN} -v ./$@/${SERVER_DIR}
	$(SETENV) env GOOS=${os} $(SETENV) env GOARCH=${arch} go build -o ${BIN_DIR}/$@/${CLIENT_BIN} -v ./$@/${CLIENT_DIR}
